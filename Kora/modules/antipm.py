...
from Kora import (COUNT_PM, CMD_HELP, BOTLOG, PM_AUTO_BAN,
                     LASTMSG, LOGGER, bot)
...
async def permitpm(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from Kora.sql.antipm_sql import is_approved
            from Kora.sql.globals import gvarstatus
        ...
        if not apprv and event.text != UNAPPROVED_MSG:
            ...
            if COUNT_PM[event.chat_id] > 4:
                ...
                try:
                    ...
                    LOGGER.info("CountPM wen't rarted boi")
                    return
...
async def auto_accept(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from Kora.sql.antipm_sql import is_approved
            from Kora.sql.antipm_sql import approve
            from Kora.sql.globals import gvarstatus
        except AttributeError:
            return
...
async def notifoff(noff_event):
    ...
    try:
        from Kora.sql.globals import addgvar
    except AttributeError:
...
async def notifon(non_event):
    ...
    try:
        from Kora.sql.globals import delgvar
    except AttributeError:
...
async def add_pmsg(cust_msg):
    ...
    try:
        import Kora.sql.globals as sql
    except AttributeError:
